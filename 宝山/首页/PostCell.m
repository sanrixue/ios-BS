...
- (void)setPostModel:(PostModel *)postModel {
    ...
    NSLog(@"!!!!!!%@",_postModel.icon);
    
    if ([[NSString stringWithFormat:@"%@",_postModel.icon] isEqualToString:@"<null>"]) {
    ...
    if ([[NSString stringWithFormat:@"%@",_postModel.name] isEqualToString:@"<null>"]) {
         self.name.text = @"皮皮虾";
    } else {
    ...
    self.time.text = [self compareCurrentTime:_postModel.start_time];
