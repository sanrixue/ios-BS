...
@interface PostController ()<UITableViewDelegate,UITableViewDataSource>{
...
- (void)viewDidLoad {
    ...
    //注册表格单元
    [self.tableView registerClass:[PostCell class] forCellReuseIdentifier:postIndentifier];
...
-(void)loadNewData
{
    ...
    NSString * url = [NSString stringWithFormat:Main_URL,PostAll_URL];
...
-(void)loadMoreData
{
    ...
    [manager POST:url parameters:dic progress:^(NSProgress * _Nonnull uploadProgress) {
        ...
        {
            [_array addObjectsFromArray:responseObject[@"data"]];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([[NSString stringWithFormat:@"%@",model.image] isEqualToString:@"<null>"]) {
        ...
        return 370+textSize.height-180;
    } else {
       
        
        return 370+textSize.height;
    }
